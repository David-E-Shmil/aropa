...
function doUnsupervisedAllocation( $assmt ) {
  ...
  case 'same tags':
    ...
    while( $row = $rs->fetch_assoc() )
      $es_by_tag[ strtoupper( (string) $row['tag'] ) ][] = $row['author'];
  ...
  case 'other tags':
    ...
    if( $allocs->allocationCount( ) > 0) {
      ...
      $esByTag = array( );
      while( $row = $rs->fetch_assoc() )
	$esByTag[strtoupper((string) $row['tag'])][] = $row['author'];
...
function saveNormalAllocations( ) {
  ...
  if( isset( $_REQUEST['authors'] ) ) {
    $authors = array( );
    foreach (preg_split("/[\s,;]+/", (string) $_REQUEST['authors'], 0, PREG_SPLIT_NO_EMPTY) as $who)
      $authors[] = $who;
  ...
  $data['tags'] = trim((string) $_REQUEST['tags']);
...
function normaliseGroup($gname) {
  return strtolower(trim((string) $gname));
}
...
function adjustAllocations( ) {
  ...
  if( $order == 'reviewer' ) {
    ...
    $reviewerIDs = count( $reviewers ) == 0 || count( $allocs ) == 0 ? array( ) :
      array_combine( $reviewers, array_map( $allocs->nameOfReviewer(...), $reviewers));
    ...
    $essayIDs = empty( $essays ) ? array( )
      : array_combine( $essays, array_map( $allocs->nameOfAuthor(...), $essays));
...
function saveAdjustAllocations( ) {
  ...
  if( $rhs == 'user' ) {
    ...
    if( $assmt['authorsAre'] == 'group' || $assmt['reviewersAre'] == 'group' ) {
      $rs = checked_mysql_query( "SELECT groupID, gname FROM `Groups` WHERE assmtID = $assmtID" );
      while( $row = $rs->fetch_assoc() )
	$rhsMap[ strtolower( trim((string) $row['gname'])) ] = -$row['groupID'];
    }
  }
...
function saveAllocateGroups( ) {
  ...
  if( isset( $_REQUEST['members'] ) && is_array( $_REQUEST['members'] ) ) {
    ...
    foreach( $_REQUEST['members'] as $groupID => $line )
      if( is_int( $groupID ) )
	foreach( identitiesToUserIDs( preg_split( '/[[:space:],;]/', (string) $line, -1, PREG_SPLIT_NO_EMPTY ) ) as $userID ) {
  ...
  foreach( array_keys( $nonEmptyGroups ) as $groupID ) {
    $gname = isset( $_REQUEST['gname'][ $groupID ] ) ? trim((string) $_REQUEST['gname'][ $groupID ]) : '';
    if( empty($gname) )
      $gname = "Group-$groupID";
