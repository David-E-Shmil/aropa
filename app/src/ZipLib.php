...
function gzip_compress ($data) {
    ...
    gzwrite($fp, (string) $data, strlen((string) $data));
    if (!gzclose($fp)) {
...
function gzip_uncompress ($data) {
    ...
    if (!($fp = fopen($filename, "wb")))
        trigger_error(sprintf("%s failed", 'fopen'), E_USER_ERROR);
    fwrite($fp, (string) $data, strlen((string) $data));
    if (!fclose($fp))
        trigger_error(sprintf("%s failed", 'fclose'), E_USER_ERROR);
...
function zip_deflate ($content)
{
    ...
    if (function_exists('gzencode'))
        $z = gzencode((string) $content);
    else
        $z = gzip_compress($content);
    
    // Suck OS type byte from gzip header. FIXME: this smells bad.
    
    extract(unpack("a2magic/Ccomp_type/Cflags/@9/Cos_type", (string) $z));
    ...
    $gz_header_len = 10;
    $gz_data_len = strlen((string) $z) - $gz_header_len - 8;
    if ($gz_data_len < 0)
        trigger_error("not enough gzip output?", E_USER_ERROR);
    
    extract(unpack("Vcrc32", substr((string) $z, $gz_header_len + $gz_data_len)));
    
    return array(substr((string) $z, $gz_header_len, $gz_data_len), // gzipped data
                 $crc32,		// crc
                 $os_type		// OS type
                 );
}
...
class ZipWriter
{
    function __construct ($comment = "", $zipname = "archive.zip") {
        ...
        $zipname = addslashes((string) $zipname);
	while( ob_end_clean( ) )
	  ;
  ...
  function addRegularFile ($filename, $content, $attrib = false) {
      ...
      $head = pack("vvvvvVVVvv",
                   ...
                   $size,
                   strlen((string) $filename),
                   strlen((string) $attrib['extra_field']));
      ...
      $this->offset += strlen($lheader) + strlen((string) $content);
      $this->nfiles++;
  }
...
class ZipReader
{
  ...
  function readFile () {
      ...
      if (strlen((string) $data) != $uncomp_size)
          ExitWiki(sprintf("Uncompressed size mismatch %d != %d",
                           strlen((string) $data), $uncomp_size));
...
function QuotedPrintableEncode ($string)
{
    ...
    while ($string) {
        ...
        preg_match('/^([ !-<>-~]*)(?:([!-<>-~]$)|(.))/s', (string) $string, $match);
        $quoted .= $match[1] . $match[2];
        if (!empty($match[3]))
            $quoted .= sprintf("=%02X", ord($match[3]));
        $string = substr((string) $string, strlen($match[0]));
    }
    // Split line.
...
function MimeContentTypeHeader ($type, $subtype, $params)
{
    ...
    foreach ($params as $key => $val) {
        ...
        if (!preg_match('/^' . MIME_TOKEN_REGEXP . '$/', (string) $val))
...
function MimeifyPageRevision ($revision) {
    ...
    foreach ($params as $key => $val)
        $params[$key] = rawurlencode((string) $val);
...
function ParseMimeMultipart($data, $boundary)
{
    ...
    $boundary = preg_quote((string) $boundary);
    
    while (preg_match("/^(|.*?\n)--$boundary((?:--)?)[^\n]*\n/s",
                      (string) $data, $match))
    {
        $data = substr((string) $data, strlen($match[0]));
        if ( ! isset($parts) )
...
function GenerateFootnotesFromRefs($params)
{
    ...
    foreach ($params as $p => $reference) {
            if (preg_match('/^ref([1-9][0-9]*)$/', (string) $p, $m))
                $footnotes[$m[1]] = sprintf(_("[%d] See [%s]"),
                                            $m[1], rawurldecode((string) $reference));
    }
...
function ParseMimeifiedPages ($data)
{
    ...
    $pagedata    = array();
    $versiondata = array();
    $pagedata['date'] = strtotime((string) $headers['date']);
...
// $Log: ZipLib.php,v $
// Revision 1.2  2005/03/22 08:29:19  jham005
// *** empty log message ***
//
// Revision 1.1  2005/03/21 19:12:55  jham005
// *** empty log message ***
//
