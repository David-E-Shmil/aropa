...
function downloadablesFile( &$ul, $files, $author, $identity, $allocIdx, $assmtID, $assmtName, $anonymous, $cid ) {
  ...
  foreach( $files as $f ) {
    ...
    $fname = basename( $f );
    if( $anonymous )
      $fname = preg_replace( "/" . preg_quote( (string) $identity ) . "/i", _('Author'), $fname );
...
function downloadFile( $filename, $author, $identity, $allocIdx, $assmtName, $anonymous ) {
  if( is_dir( $filename ) ) {
    ...
    if( $anonymous && ! empty($author) )
      $fname = preg_replace( "/" . preg_quote( (string) $identity ) . "/i",
			     _('Author'), $fname );
...
function downloadEssay($id, $author, $identity, $extn, $name, $allocIdx, $anonymous, $cid) {
  ...
  prepareForDownload( $extn, strlen((string) $essay), $name );
  echo $essay;
  exit;
}
...
function fileExtension($name) {
  $n = strrpos((string) $name, '.');
  return ($n === false) ? '' : substr((string) $name, $n);
}
...
function prepareForDownload( $extn, $size, $fname = null ) {
  ...
  if( ! empty( $fname ) )
    header( 'Content-Disposition: attachment; filename=' . str_replace('+', '_', urlencode( (string) $fname )) . ';' );
...
function showReviewFile( ) {
  ...
  prepareForDownload( $row['extn'], strlen((string) $review), $row['description'] ? $row['description'] : 'untitled' );
  echo $review;
  exit;
}
