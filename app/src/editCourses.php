...
function saveClasses( ) {
  ...
  foreach( $_REQUEST['cname'] as $courseID => $cname ) {
    ...
    foreach( identitiesToUserIDs( preg_split( "/[ ,;\t]/", (string) $_REQUEST['instruct'][ $courseID ], -1, PREG_SPLIT_NO_EMPTY ) )
	     as $userID )
...
function editClass( ) {
  ...
  if( ! empty( $_REQUEST['cname'] ) ) {
    ...
    if( $_REQUEST['courseID'] == 0 ) {
      ...
      $fields = array('cname'   => trim((string) $_REQUEST['cname']),
		      'cident'  => trim((string) $_REQUEST['cident']),
