...
class GradeCalc {
  ...
  function dumpToFile( $assmtName, $filename, $ndp = 2 ) {
    ...
    header( 'Content-Disposition: attachment; filename=' . rawurlencode((string) $filename) . ';' );
  ...
  function commentStats( $r, $stat = 'all' ) {
	...
	$t = strip_tags((string) $row['comments']);
  ...
  */
  ...
  function JSON_reviewers( ) {
    ...
    while( $row = $rs->fetch_assoc() ) {
      $madeBy = $row['madeBy'];
      $ws = str_word_count(strip_tags((string) $row['comments']), 1);
...
function jsonGrades( ) {
  ...
  if( isset( $_REQUEST['excl'] ) )
    $g['ignore'] = array_unique( array_merge( $g['ignore'], explode( ',', (string) $_REQUEST['excl'] )));

  if( isset( $_REQUEST['incl'] ) )
    $g['ignore'] = array_diff( $g['ignore'], explode( ',', (string) $_REQUEST['incl'] ));
