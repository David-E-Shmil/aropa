...
function dumpTable( $table, $cond, &$map, $key = null ) {
  ...
  while( $row = $rs->fetch_assoc() ) {
	...
	$v[] = 'NULL';
      else if( isBlobby( $colType[ $f ] ) && strlen( (string) $data ) > $MAX_SQL_PACKET ) {
	...
	for( $start = 0; $start < strlen( (string) $data ); $start += $MAX_SQL_PACKET )
	  echo "UPDATE $table SET $f = CONCAT($f,\n"
	    . quote_smart( substr( (string) $data, $start, $MAX_SQL_PACKET ) )
