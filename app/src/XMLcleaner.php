...
class XMLcleaner {
  ...
  function defaultHandler( $parser, $data ) {
    global $validEntities;
    if( substr((string) $data, 0, 1) == "&"
        && substr((string) $data, -1, 1) == ";"
        && in_array( substr((string) $data, 1, -1), $validEntities )
        )
      $this->cleanXML .= $data;
    return true;
  }
  ...
  */
  ...
  function clean( $data ) {
    ...
    if( !xml_parse( $parser, preg_replace('/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]/', '', (string) $data ), true ) ) {
...
function declareEntities( $data ) {
  $entityDecl = "";
  for( $pos = strpos( (string) $data, '&', 0 )
         ; $pos !== false
         ; $pos = strpos( (string) $data, '&', $pos+1 )
       ) {
    $semi = strpos( (string) $data, ';', $pos+1 );
    if( $semi !== false ) {
      global $validEntities;
      $entity = substr( (string) $data, $pos+1, $semi-$pos-1 );
