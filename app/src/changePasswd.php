...
function savePassword() {
  ...
  if(! $revert && ! isset($_SESSION['tmpLogin']) && !password_verify((string) $oldpassword, (string) $acct['passwd'])) {
    addWarningMessage(_('You did not enter your old password correctly'));
    redirect('changePassword');
  }
      
  $passwd = empty($password) ? '' : password_hash((string) $password, PASSWORD_DEFAULT);
