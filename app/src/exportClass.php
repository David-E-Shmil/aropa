  ...
  function __construct( $prefix, $instID = null ) {
    ...
    if( $prefix )
      ...
      $this->_offset = intval( preg_replace( '/.*\D(\d+)$/', '$1',
					     (string) fetchOne( "SELECT MAX(uident) AS u FROM User WHERE instID=$instID AND uident RLIKE "
						       . quote_smart('^' . preg_quote((string) $prefix) . '[[:digit:]]+$'), 'u' ) ));
...
function doImportClass( ) {
  ...
  if( trim((string) $cname) == '' ) $cname = "".$xml->name;
...
function addAssigmentDetails( $xml, $assmt ) {
  ...
  if( ! empty( $assmt['visibleReviewers'] ) ) {
    ...
    foreach( preg_split("/[ \t,;]+/", (string) $assmt['visibleReviewers'], -1, PREG_SPLIT_NO_EMPTY) as $who )
...
function addEssays( $xml, $assmtID ) {
  ...
  while( $row = $rs->fetch_assoc() ) {
	...
	$data .= $dd['data'];
      */
      $xe = $xs->addChild('essay', bin2hex((string) $data));
