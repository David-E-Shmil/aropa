...
function updateEssay( ) {
  ...
  if (isset($_POST['comment'])) {
    ...
    foreach ($_POST['comment'] as $item => $c)
      if (isset($assmt['comment-items'][$item]) && trim((string) $c) != "")
	$commentsPosted[$item] = preg_replace('/(<|&lt;)!--.*?--(>|&gt;)/', '', (string) $c);
  }
  ...
  if( isset( $_FILES['file']['error'] ) ) {
    ...
    foreach( $_FILES['file']['error'] as $item => $error ) {
      ...
      if( ! $tmp )
	continue;
      $info = pathinfo( (string) $_FILES['file']['name'][ $item ] );
    ...
    foreach( $upload as $item => $u ) {
      if( strlen( (string) $u['contents'] ) > $MAX_CHUNK ) {
	$overflow = substr( (string) $u['contents'], $MAX_CHUNK );
	$u['contents'] = substr( (string) $u['contents'], 0, $MAX_CHUNK );
	$u['overflow'] = true;
      } else
	$overflow = '';
