...
function login( ) {
  ...
  if( isLTISessionRequest( ) ) {
	...
	if (!$acct && !empty($_POST['lis_person_contact_email_primary'])) {
	  ...
	  if( ! $userID ) {
	    $p0 = strpos( (string) $email, '@' );
	    if( $p0 !== false ) {
	      $uident = trim( substr( (string) $email, 0, $p0 ) );
	      if( ! empty( $uident ) )
...
function checkPassword( $instID, $features, $username, $password, $validated ) {
  ...
  if ($validated == 'tmp'
      ...
      && strtotime((string) $acct['lastChanged']) > time() - 180 * 24 * 60 * 60) // Last set a password within the past 6 months or so
  ...
  if ($validated === 'ok')
    ...
    checked_mysql_query('update User set passwd = ' . quote_smart(password_hash((string) $password, PASSWORD_DEFAULT))
			. ", lastChanged = now()"
...
function checkPassword2( $instID, $features, $username, $password, $acct ) {
  if (!empty($acct['passwd']) &&
      str_starts_with((string) $acct['passwd'], '$1$') &&
      $acct['passwd'] == $crypt)
    return 'ok';

  if (!empty($acct['passwd']) && password_verify((string) $password, (string) $acct['passwd']))
    return 'ok';
  ...
  if( ! empty( $features['LDAP_SERVER'] )
    ...
    if( $ld ) {
      foreach( explode( ";", (string) $features['LDAP_BASE_DN'] ) as $base_dn )
	if( strpos( $base_dn, '$USER' ) !== false ) {
	  //- $user character string escape are from RFC2253, section 2.4
	  $rdn = str_replace( '$USER', addcslashes( (string) $username, ',# +";\\' ), $base_dn );
...
function loginUser( $instID, $userID, $status, $uident, $username, $prefs, $timezone ) {
  ...
  preg_match("/([+-])?(\d\d):(\d\d):\d\d/", (string) fetchOne('select timediff(now(), utc_timestamp) as t', 't'), $tz);
  if (count($tz) == 4) {
    if ($tz[1] == '') $tz[1] = '+';
