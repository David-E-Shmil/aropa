...
function addExtensions( ) {
  ...
  if( strstr( (string) $assmt['allocationType'], 'tag' ) !== false ) {
    ...
    $tags = preg_split("/[;,[:space:]]/", (string) $assmt['tags'], -1, PREG_SPLIT_NO_EMPTY);
  } else
    $tagMessage = '';
...
function saveExtensions( ) {
  ...
  $rs = checked_mysql_query( 'select author, uident'
			     ...
			     . " where assmtID = $assmtID" );
  while( $row = $rs->fetch_assoc() )
    $authorList[ strtolower((string) $row['uident']) ] = $row['author'];
    
  $rs = checked_mysql_query( 'select reviewer, uident'
			     . ' from Reviewer inner join User on reviewer = userID'
			     . " where assmtID = $assmtID" );
  while( $row = $rs->fetch_assoc() )
    $reviewList[ strtolower((string) $row['uident']) ] = $row['reviewer'];
  ...
  if( $assmt['authorsAre'] != 'other' || $assmt['reviewersAre'] != 'other' ) {
	...
	$authorList[ strtolower((string) $row['uident']) ] = $row['userID'];
      if( $assmt['reviewersAre'] != 'other' )
	$reviewList[ strtolower((string) $row['uident']) ] = $row['userID'];
    }
  }
  ...
  foreach( $_REQUEST['uident'] as $n => $uident ) {
    $uident = strtolower((string) normaliseUident($uident));
    if( empty( $uident ) )
      continue;
