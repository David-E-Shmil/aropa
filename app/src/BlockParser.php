...
class RubricTransform {
  ...
  function transform( ) {
	...
	$this->recentLabel = preg_replace('/([^?!.]*.).*/', '\\1', (string) $node->wholeText);

      switch( $node->nodeName ) {
      case "img":
	$src = $node->getAttribute("src");
	if( $src )
	  if( stripos( (string) $src, "/textBlock.jpg" ) !== false ) {
  ...
  function fixupImagePath( $src, $node ) {
    if( strpos( (string) $src, '$AUTHOR' ) !== false || strpos( (string) $src, '$REVIEWER' ) !== false )
  ...
  function makeComment($node) {
	...
	$height = (int)$node->getAttribute('height');
      else if (preg_match('/height\s*:\s*[\'"]?\s*(\d+)\s*(pt|pc|em|ex|px)?/', (string) $style, $matches))
	$height = cssUnitsToPt($matches[1], $matches[2]);

      if ($node->hasAttribute('width'))
	$width = (int)$node->getAttribute('width');
      else if (preg_match('/width\s*:\s*[\'"]?\s*(\d+)\s*(pt|pc|em|ex|px)?/', (string) $style, $matches))
...
function TransformRubric( $rubric, $marks = array(), $comments = array(), $reviewFiles = array( ), $edit = 'no edit', $alloc = array() ) {
  $rubric = preg_replace('/(<|&lt;)!--.*?--(>|&gt;)/', '', (string) $rubric);
...
function isHTML( $str ) {
  return strpos( (string) $str, '<' ) == 0          //- Starts with <
    || strpos( (string) $str, '</'     ) !== false  //- or contains one of these
    || strpos( (string) $str, '&nbsp;' ) !== false
    || strpos( (string) $str, '&quot;' ) !== false
    || strpos( (string) $str, '&#39;'  ) !== false
    || strpos( (string) $str, '<br />' ) !== false
    || strpos( (string) $str, '<img'   ) !== false
  ;
}
