...
function adjustDates( &$row, $dateDiff, $fields ) {
  if( $dateDiff )
	...
	$row[ $dateField ] = date('Y-m-d H:i:s', strtotime((string) $row[$dateField]) + $dateDiff);
}
...
function cloneAsTestClassInternal($baseCourseID, $instID, $cname, $owner, $cident, $subject, $unamePat, $dateShift ) {
  ...
  $offset = intval( preg_replace( '/.*\D(\d+)$/', '$1',
				  (string) fetchOne( "SELECT MAX(uident) AS u FROM User WHERE instID=$instID AND uident RLIKE "
					    . quote_smart('^' . preg_quote((string) $unamePat) . '[[:digit:]]+$'), 'u' ) ));
  ...
  foreach( fetchAll( "SELECT * FROM Assignment WHERE courseID=$baseCourseID ORDER BY submissionEnd ASC" ) as $assmt ) {
    ...
    case 'pre':
      $dateDiff = strtotime('+ 1 day') - strtotime((string) $assmt['submissionEnd']);
      break;
    case 'mid':
      $dateDiff = strtotime('- 1 day') - strtotime((string) $assmt['submissionEnd']);
      break;
    case 'post':
      $dateDiff = strtotime('- 1 day') - strtotime((string) $assmt['reviewEnd']);
      break;
    default:
      $dateDiff = null;
    }
