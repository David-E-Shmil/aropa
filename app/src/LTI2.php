...
function getRequestURL( ) {
  ...
  if( $_SERVER['SERVER_PORT'] != $port )
    $url = "$pageURL://" . strtolower( (string) $_SERVER['SERVER_NAME'] ) . ":$_SERVER[SERVER_PORT]$_SERVER[REQUEST_URI]";
  else
    $url = "$pageURL://" . strtolower( (string) $_SERVER['SERVER_NAME'] ) . "$_SERVER[REQUEST_URI]";
  return $url;
}

function getOAuthSignature( $params, $endpoint, $method, $oauth_consumer_secret ) {
  if( strpos( (string) $endpoint, '?' ) !== false ) {
    // Add any GET params to the OAuth parameters for signing.
    list( $endpoint, $getparams ) = explode( '?', (string) $endpoint, 2 );
  ...
  $parts = OAuthUtil::urlencode_rfc3986( array( strtoupper((string) $method), $endpoint, $params ) );

  $args = array(  );
  foreach( $params as $k => $v )
      $args[] = rfc3986encode( $k ) . '=' . rfc3986encode( $v );
  $basestring =  strtoupper((string) $method). '&' . rfc3986encode($endpoint) . '&' . rfc3986encode( join( '&', $args ) );
...
function rfc3986encode( $input ) {
  ...
  return str_replace('%7E', '~', rawurlencode((string) $input));
}
...
class OAuthSignatureMethod_HMAC_SHA1 extends OAuthSignatureMethod {
  ...
  public function build_signature($request, $consumer, $token) {
    ...
    $computed_signature = base64_encode(hash_hmac('sha1', (string) $base_string, $key, true));
...
class OAuthSignatureMethod_RSA_SHA1 extends OAuthSignatureMethod {
  ...
  protected function fetch_public_cert(&$request): never {
  ...
  protected function fetch_private_cert(&$request): never {
    // not implemented yet, ideas are:
    // (1) do a lookup in a table of trusted certs keyed off of consumer
    //
    // either way should return a string representation of the certificate
...
class OAuthRequest {
  ...
  public function to_header() {
    ...
    foreach ($this->parameters as $k => $v) {
      if (substr((string) $k, 0, 5) != "oauth") continue;
      if (is_array($v)) {
...
class TrivialOAuthDataStore extends OAuthDataStore {
    ...
    function lookup_consumer($consumer_key) {
        if ( strpos((string) $consumer_key, "http://" ) === 0 ) {
...
function toolState() {
  $fields = array('accept', 'accept-encoding', 'accept-language', 'host', 'user-agent');
  $sig = '';
  foreach (apache_request_headers() as $header => $value)
    if (in_array(strtolower($header), $fields))
      $sig .= $value;
  // TODO: include $_COOKIE values in $sig
