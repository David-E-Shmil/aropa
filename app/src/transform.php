...
function SetHTMLOutputMode( $tag, $tagtype, $level ) {
  ...
  if( $tagtype == ZERO_LEVEL ) {
	...
	$stack->push($tag);
      ...
      if ($stack->cnt() == 1 and
	  preg_match('/^(p|pre|h\d)$/i', (string) $stack->top()))
	{
	  $closetag = $stack->pop();
	  $retvar .= "</$closetag>";
	}
...
function PHPwikiToXml( $wiki ) {
  ...
  foreach( preg_split( "/[\n]/", (string) $wiki ) as $wikiline ) {
    ...
    // bold (old way)
    $wikiline = preg_replace("|(''')(.*?)(''')|",
			     "<strong>\\2</strong>", (string) $wikiline);
    ...
    // italics
    $wikiline = preg_replace("|('')(.*?)('')|",
			     "<em>\\2</em>", (string) $wikiline);
    $wikiline = preg_replace("|(_)(.*?)(_)|",
			     "<em>\\2</em>", (string) $wikiline);

    /* [Aropa markup */
    $wikiline = preg_replace("/{img\s*([^}]+)}/",
			     "<img src='\\1'>", (string) $wikiline);
    $wikiline = preg_replace("/{-\s*\(\s*(\d+)\s*,\s*(\d+)\s*\)([^}]*)-}/e",
			     "'\\3<br/><img src=\'tinymce/jscripts/tiny_mce/plugins/aropa/img/textBlock.jpg\'"
			     . " style=\'height:' . (2*$1) . 'em; width:\\2ex\' name=\'' . \$context . '\' />'", (string) $wikiline );
    $wikiline = preg_replace("/{-([^}]*)-}/e",
			     "'\\1<br/><img src=\'tinymce/jscripts/tiny_mce/plugins/aropa/img/textBlock.jpg\'/>'", (string) $wikiline );
    $wikiline = preg_replace("/{([^}]*)=([^}]*)}/e",
			     "'<span><input type=\\'radio\\'/></span>'", (string) $wikiline);
    $wikiline = preg_replace("/{([^}]*)\\*([^}]*)}/e",
			     "'<input type=\\'checkbox\\'/>'", (string) $wikiline);
    // $wikiline = preg_replace("/{-([^}]*)-}/e",
    ...
    $wikiline = preg_replace("/{([^}]*)}/e",
    			     "setContext(\"\\1\")", (string) $wikiline );

    if (preg_match("/^(.*):\s*$/", (string) $wikiline, $matches)) {
      $html .= SetHTMLOutputMode('dl', NESTED_LEVEL, 1);
      $html .= '<dt>' . $matches[1] . '</dt>';
      $wikiline = '';
    } elseif( $stack->bot() == 'dl' && preg_match( "/^\s+(.*)/", (string) $wikiline, $matches ) ) {
      $html .= SetHTMLOutputMode('dd', NESTED_LEVEL, 2 );
      $html .= $matches[1];
      $wikiline = '';
    } elseif( preg_match( "/^\s*<\s*pre\s*>/", (string) $wikiline ) ) {
      $html .= SetHTMLOutputMode('pre', ZERO_LEVEL, 0);
      $wikiline = '';
    } elseif( preg_match( "|^\s*<\s*/\s*pre\s*>|", (string) $wikiline ) ) {
      $html .= SetHTMLOutputMode('p', ZERO_LEVEL, 0);
      $wikiline = '';
    } elseif( $stack->bot() == 'pre' ) {
      $wikiline = htmlspecialchars( (string) $wikiline );
    } else
    /* Aropa markup] */ 
    ...
    if (preg_match("/(^\t+)(.*?)(:\t)(.*$)/", (string) $wikiline, $matches)) {
    ...
    } elseif (preg_match("/(^\t+)(\*|\d+|#)/", (string) $wikiline, $matches)) {
	...
	$listtag = 'ol'; // a rather tacit assumption. oh well.
      ...
      $wikiline = preg_replace("/^(\t+)(\*|\d+|#)/", "", (string) $wikiline);
      ...
      // unordered lists <UL>: "*"
    } elseif (preg_match("/^([#*]*\*)[^#]/", (string) $wikiline, $matches)) {
      // this is part of an unordered list
      $numtabs = strlen($matches[1]);
      $wikiline = preg_replace("/^([#*]*\*)/", '', (string) $wikiline);
	...
	$wikiline = '<dt>' . $matches[2];
      ...
      $html .= SetHTMLOutputMode('pre', ZERO_LEVEL, 0);
      */
    } elseif (preg_match("/^\s*(!{1,3})[^!]/", (string) $wikiline, $whichheading)) {
    ...
    } elseif (preg_match('/^-{4,}\s*(.*?)\s*$/', (string) $wikiline, $matches)) {
