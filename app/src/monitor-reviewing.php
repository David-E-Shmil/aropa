...
function viewSubmissions( ) {
  ...
  $requirements = array();
  foreach (explode("\n", (string) $assmt['submissionRequirements']) as $requireStr) {
...
function group_ASC( $a, $b ) {
  if( preg_match( '/^Group-(\d+)$/', (string) $a[0]['gname'], $na ) == 1 && preg_match( '/^Group-(\d+)$/', (string) $b[0]['gname'], $nb ) == 1 )
    return (int)$na[1] - (int)$nb[1];
  else
    return strcasecmp( (string) $a[0]['gname'], (string) $b[0]['gname'] );
}
function whenUploaded_ASC( $a, $b ) {
  ...
  return strcmp((string) $maxA, (string) $maxB);
}
function tag_ASC( $a, $b ) {
  return strcasecmp( (string) $a[0]['tag'], (string) $b[0]['tag'] );
}
...
function essayCell( $essays, $reqIndex, $req, $cid ) {
  foreach( $essays as $e )
    if( $e['reqIndex'] == $reqIndex ) {
      ...
      switch( $req[0] ) {
      case 'file':
	if( preg_match( makeExtnPREG( $args['extn'], $args['other'] ), (string) $e['description'] ) > 0 )
	  $desc = $e['description'];
	else
...
function downloadByID( ) {
  ...
  if ($row['extn'] == 'inline-text') {
    ...
    prepareForDownload($row['extn'], strlen((string) $essay), $row['description']);
    echo $essay;
    exit;
  }
}
...
function makeExtnPREG( $extn, $other ) {
  ...
  default:
    $pat = array( );
    foreach( explode(",", (string) $extn ) as $e )
      $pat[] = '(' . preg_quote($e) . ')';
    return "/\\." . join('|', $pat) . '$/i';
  }
}
...
function essayTagCell( $essays, $assmtTags ) {
  ...
  if( ! $tag )
    ...
    foreach( $assmtTags as $t )
      if( strcasecmp( (string) $e['tag'], (string) $t ) == 0 ) {
	$found = true;
	break;
      }
    if( ! $found )
...
function viewAllocations( ) {
  ...
  case 'reviewer':
    $buttons->pushContent(RedirectButton(_('View by author'),
					 "viewAllocations&assmtID=$assmtID&cid=$cid&view=submission&tag=" . rawurlencode((string) $tag)));
    $table = $allocs->showByReviewer( $cid, false );
    break;
  case 'submission':
    $buttons->pushContent(RedirectButton(_('View by reviewer'),
					 "viewAllocations&assmtID=$assmtID&cid=$cid&view=reviewer&tag=" . rawurlencode((string) $tag)));
...
function downloadReviewing( ) {
  ...
  if( $view == 'reviewer' ) {
    ...
    $reviewerIDs = array_combine( $reviewers, array_map( $allocs->nameOfReviewer(...), $reviewers));
    asort( $reviewerIDs );
    ...
    $authorIDs = array_combine( $essays, array_map( $allocs->nameOfAuthor(...), $essays));
    asort( $authorIDs );
...
function reviewFileUL($assmt, $cid, $allocID) {
  ...
  if (isset($assmt['nReviewFiles']) && $assmt['nReviewFiles'] > 0) {
    ...
    while ($rf = $rfs->fetch_assoc()) {
      ...
      $desc = trim((string) $rf['description']);
