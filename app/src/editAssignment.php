...
function editAssignmentCommon( &$assmt, $assmtID, $cid ) {
  ...
  $spec = array( );
  $typesUsed = array();
  foreach( preg_split( "/\n/", (string) $assmt['submissionRequirements'], -1, PREG_SPLIT_NO_EMPTY) as $line ) {
...
function saveAssignment( ) {
  ...
  if( isset( $_REQUEST['basepath'] ) )
    $fields['basepath']        = trim( (string) $_REQUEST['basepath'] );
  ...
  $values = array( );
  $visibleReviewerIDs = identitiesToUserIDs( preg_split("/[ \t,;]+/", (string) $fields['visibleReviewers'], -1, PREG_SPLIT_NO_EMPTY) );
