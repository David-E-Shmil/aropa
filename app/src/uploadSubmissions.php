...
function upload( ) {
  ...
  $tags = array( );
  if( strstr( (string) $assmt['allocationType'], 'tag' ) !== false )
    foreach( preg_split("/[;,[:space:]]/", (string) $assmt['tags'], -1, PREG_SPLIT_NO_EMPTY) as $tag )
      $tags[ trim($tag) ] = true;
...
return true;
		      ...
		      $tagSelector);
  $reqIndex = 1;
  foreach( preg_split( "/\n/", (string) $assmt['submissionRequirements'], -1, PREG_SPLIT_NO_EMPTY ) as $requireStr ) {
...
function makeTagSelect( $tags, $defaultTag ) {
  ...
  foreach( $tags as $t )
    $select->pushContent( HTML::option(array('selected'=> strcasecmp((string) $t, (string) $defaultTag) == 0 ), $t) );
  return $select;
}
...
function saveUploads( ) {
  ...
  if( isset( $_FILES['file']['error'] ) )
    foreach( $_FILES['file']['error'] as $reqIndex => $error ) {
      ...
      if( ! $tmp )
	continue;
      $info = pathinfo( (string) $_FILES['file']['name'][ $reqIndex ] );
  ...
  if( isset( $_REQUEST['text'] ) )
    foreach( $_REQUEST['text'] as $reqIndex => $text ) {
      $words = str_word_count( (string) preg_replace( '/&\w+;/', '', str_replace( '&nbsp;', ' ', strip_tags((string) $text) )));
      if( $words > 0 ) {
  ...
  if( stripos( (string) $assmt['allocationType'], 'tag' ) !== false ) {
    // The tag must be in $assmt['tags']
    $assmtTags = preg_split("/[,;[:space:]]+/", (string) $assmt['tags'], -1, PREG_SPLIT_NO_EMPTY);
  ...
  foreach( $upload as $reqIndex => $u ) {
    if( strlen( (string) $u['essay'] ) > $MAX_CHUNK ) {
      $overflow = substr( (string) $u['essay'], $MAX_CHUNK );
      $u['essay'] = substr( (string) $u['essay'], 0, $MAX_CHUNK );
      $u['overflow'] = true;
    } else
      $overflow = '';
