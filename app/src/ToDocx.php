...
function WriteReviewsToDocx( $assmt, $cid, $allocs, $selectedAuthor, $selectedReviewer, $template, $stylesheet ) {
  ...
  $feedback->appendChild($xml->createElement('assignment', htmlspecialchars((string) $assmt['aname'])));
  $feedback->appendChild($xml->createElement('class', htmlspecialchars((string) className($cid))));
