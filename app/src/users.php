...
function normaliseUident( $username, $instID = null ) {
  ...
  $uident = strtolower(trim((string) $username));
  if (institutionHasFeature('UIDENT_REGEX', $instID)) {
	...
	if( $i > 0 && ! empty( $match ) ) {
	  if( institutionHasFeature( 'UIDENT_FORMAT', $instID ) ) {
	    $fmts = explode( ';',  (string) $inst->features['UIDENT_FORMAT'] );
...
function checkNames() {
  ...
  $names = array( );
  foreach( preg_split( '/[[:space:],;]/', (string) $_REQUEST['names'], -1, PREG_SPLIT_NO_EMPTY ) as $name )
  ...
  foreach( fetchAll( 'SELECT uident FROM UserCourse uc LEFT JOIN User u ON uc.userID = u.userID'
		     ...
		     'uident' ) as $uident ) {
    $uident = strtolower( (string) $uident );
    unset( $names[ $uident ] );
  }
