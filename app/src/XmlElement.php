...
class XmlElement extends XmlContent
{
	...
	$this->_parseClasses();
	return isset($this->_classes[trim((string) $class)]);
    }
...
//-]
    ...
    function emptyTag() {
        return substr((string) $this->startTag(), 0, -1) . "/>";
    }
...
class FormattedText {
    ...
    function _init ($args) {
        ...
        $m = array();
        if (! preg_match_all('/(?<!%)%(\d+)\$/x', (string) $this->_fs, $m)) {
            $this->_args  = $args;
        }
        else {
            // Format string has '%2$s' style argument reordering.
            // PHP doesn't support this.
            if (preg_match('/(?<!%)%[- ]?\d*[^- \d$]/x', (string) $this->_fs)) // $fmt
            ...
            $this->_fs = preg_replace('/(?<!%)%\d+\$/x', '%', (string) $this->_fs);
