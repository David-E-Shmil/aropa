...
function rubricLabels( $cid, $id, $rubricType, $caller ) {
  ...
  if( isset( $_REQUEST['i'] ) || isset( $_REQUEST['g'] ) || isset( $_REQUEST['c'] ) ) {
	...
	for( $item = 0; $item < $nRadios; $item++ ) {
	  $value = trim((string) $_REQUEST['i'][$radioID][$item]);
...
function rawRubric( ) {
  ...
  header('Content-Length: ' . strlen((string) $rubric) );
...
function sameXML( $xmlA, $xmlB ) {
  ...
  if( $xmlA->nodeType == XML_TEXT_NODE )
    if( trim(preg_replace('/\s\s+/', ' ', (string) $xmlA->wholeText)) != trim(preg_replace('/\s\s+/', ' ', (string) $xmlB->wholeText)) )
      return false;
...
function skipEmptyNodes( $node ) {
  while( $node && $node->nodeType == XML_TEXT_NODE && trim((string) $node->wholeText) == '' )
